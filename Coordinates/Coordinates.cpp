...
Coordinates::Cooridinates(geo_location width,geo_location length):width(width),length(length){
width.total_deegres_value=width.degrees+float(width.minutes)/60+float(width.seconds)/3600;
ength.total_deegres_value=length.degrees+float(length.minutes)/60+float(length.seconds)/3600;
}

Coordinates::distanceFrom(Coordinates differentCity){
	float distance=0;
